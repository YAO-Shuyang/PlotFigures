...
def get_data(mice: int, maze_type: int, dates: list):
    ...
    if os.path.exists(os.path.join(data_loc, str(mice)+'-'+str(maze_type)+'-op shuffle.pkl')):
        with open(os.path.join(data_loc, str(mice)+'-'+str(maze_type)+'-op shuffle.pkl'), 'rb') as handle:
            DataOP = pickle.load(handle)
    else:
        DataOP = np.zeros((len(dates), 144), dtype=np.float64)
        
        for d in range(len(dates)):
            i = np.where((f1['date']==dates[d])&(f1['maze_type']==0)&(f1['MiceID']==mice))[0][0]
        
            with open(f1['Trace File'][i], 'rb') as handle:
                trace = pickle.load(handle)

            DataOP[d, :] = field_arange(trace)
        
        with open(os.path.join(data_loc, str(mice)+'-'+str(maze_type)+'-op shuffle.pkl'), 'wb') as f:
            pickle.dump(DataOP, f)
...
Data, DataOP = get_data(11095, 1, dates)
Data = (Data.T / np.nansum(Data, axis = 1)).T * 100
DataOP = (DataOP.T / np.nansum(DataOP, axis = 1)).T * 100
print("Plot 11095, Maze 1")
labels = ['Day 4', 'Day 5', 'Day 6', 
          'Day 7', 'Day 8', 'Day 9']
FieldDisImage.plot_with_open_field(maze_type=1, data=Data, 
                                   save_loc=loc, file_name='Maze 1-11095 [Compare with op]',
                                   open_field=DataOP, 
...
dates = [
         20220820, 20220822, 20220824, 20220826, 20220828, 20220830]
Data, DataOP = get_data(11095, 2, dates)
Data = (Data.T / np.nansum(Data, axis = 1)).T * 100
DataOP = (DataOP.T / np.nansum(DataOP, axis = 1)).T * 100
print("Plot 11095, Maze 2")
labels = ['Day 4', 'Day 5', 'Day 6', 
          'Day 7', 'Day 8', 'Day 9']
FieldDisImage.plot_with_open_field(maze_type=2, data=Data, 
                                   save_loc=loc, file_name='Maze 2-11095 [Compare with op]',
                                   open_field=DataOP, 
                                   cp_args = {'labels':labels},
                                   ip_args = {})
# plot_field_arange_all(Data, maze_type=1, save_loc=loc, file_name='Maze 1-11095', labels=labels, title='Maze 1, Mouse #11095')
...
dates = [
         20220820, 20220822, 20220824, 20220826, 20220828, 20220830]
Data, DataOP = get_data(11092, 1, dates)
Data = (Data.T / np.nansum(Data, axis = 1)).T * 100
DataOP = (DataOP.T / np.nansum(DataOP, axis = 1)).T * 100
...
labels = ['Day 4', 'Day 5', 'Day 6', 
          'Day 7', 'Day 8', 'Day 9']
FieldDisImage.plot_with_open_field(maze_type=1, data=Data, 
                                   save_loc=loc, file_name='Maze 1-11092 [Compare with op]',
                                   open_field=DataOP, 
                                   cp_args = {'labels':labels},
                                   ip_args = {})
# plot_field_arange_all(Data, maze_type=1, save_loc=loc, file_name='Maze 1-11095', labels=labels, title='Maze 1, Mouse #11095')
...
dates = [
         20220820, 20220822, 20220824, 20220826, 20220828, 20220830]
Data, DataOP = get_data(11092, 2, dates)
Data = (Data.T / np.nansum(Data, axis = 1)).T * 100
DataOP = (DataOP.T / np.nansum(DataOP, axis = 1)).T * 100
...
labels = ['Day 4', 'Day 5', 'Day 6', 
          'Day 7', 'Day 8', 'Day 9']
FieldDisImage.plot_with_open_field(maze_type=2, data=Data, 
                                   save_loc=loc, file_name='Maze 2-11092 [Compare with op]',
                                   open_field=DataOP, 
                                   cp_args = {'labels':labels},
                                   ip_args = {})
# plot_field_arange_all(Data, maze_type=1, save_loc=loc, file_name='Maze 1-11095', labels=labels, title='Maze 1, Mouse #11095')
