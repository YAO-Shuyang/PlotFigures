...
if os.path.exists(os.path.join(figdata, code_id+' - Test Against Normal Distribution.pkl')) == False:
    Data = DataFrameEstablish(variable_names = ['Statistic', 'PValue'], f = f1, 
                              function = KSTestNormal_Interface, func_kwgs = {'is_placecell': True},
                              file_name = code_id+" - Test Against Normal Distribution", behavior_paradigm = 'CrossMaze')
else:
    with open(os.path.join(figdata, code_id+' - Test Against Normal Distribution.pkl'), 'rb') as handle:
...
plt.tight_layout()
plt.savefig(os.path.join(loc, "PValue-KSTest [Normal]"+'.png'), dpi = 2400)
plt.savefig(os.path.join(loc, "PValue-KSTest [Normal]"+'.svg'), dpi = 2400)
plt.close()