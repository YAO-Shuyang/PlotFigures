...
ax.set_ylim(0, 1)
ax.set_yticks(np.linspace(0, 1, 6))
plt.savefig(join(loc, "[Maze 1 10212] Real Survival Frac. over time.png"), dpi=600)
plt.savefig(join(loc, "[Maze 1 10212] Real Survival Frac. over time.svg"), dpi=600)
plt.show()
...
ax.set_ylim(0, 1)
ax.set_yticks(np.linspace(0, 1, 6))
plt.savefig(join(loc, "[Maze 1 10209] Real Survival Frac. over time.png"), dpi=600)
plt.savefig(join(loc, "[Maze 1 10209] Real Survival Frac. over time.svg"), dpi=600)
plt.show()
...
idx = np.where((Data['Drift Type'] == 'Converged')&(Data['Session Interval'] + Data['Start Session'] <= 14))[0]
...
idx = np.where((Data['Drift Type'] == 'Converged Poly')&(Data['Session Interval'] + Data['Start Session'] <= 14))[0]
SubData = SubDict(Data, Data.keys(), idx)
fig = plt.figure(figsize=(4,2))
ax = Clear_Axes(plt.axes(), close_spines=['top', 'right'], ifxticks=True, ifyticks=True)
sns.lineplot(
...
for i in range(5):
    ...
    idx = np.where((SubData['MiceID'] > 50+i*10)&(SubData['MiceID'] <= 60+i*10)&(SubData['Session Interval'] + SubData['Start Session'] <= 14))[0]
    sns.lineplot(
