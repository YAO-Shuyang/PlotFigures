...
loc = join(figpath, code_id, 'with distance')
mkdir(loc)

idx = np.where(f_CellReg_modi['Type'] == 'Real')[0]
if os.path.exists(join(figdata, code_id+' [with distance].pkl')):
    with open(join(figdata, code_id+' [with distance].pkl'), 'rb') as handle:
    ...
    Data = DataFrameEstablish(
        ...
        file_name = code_id+ ' [with distance]', file_idx=idx,
