...
code_id = "0405 - Stability vs Linearized Position"
...
if os.path.exists(join(figdata, code_id+' [spearman].pkl')) == False:
    ...
    for i in tqdm(range(PData['Poisson KS P-Value'].shape[0])):
        ...
        x, y = cp.deepcopy(SubData['Position']), cp.deepcopy(SubData['FSC Stability'])
...
if os.path.exists(os.path.join(figdata, code_id+'.pkl')) == False:
    IndeptData = DataFrameEstablish(variable_names = ['Statistic', 'P-Value', 
                              ...
                              function = IndeptTestForPositionAndStability_Interface,
...
for i in range(PData['Poisson KS P-Value'].shape[0]):
    ...
    ax.plot(SubData['Position'], SubData['FSC Stability'], 'o', color='black', markeredgewidth=0, markersize=2)
