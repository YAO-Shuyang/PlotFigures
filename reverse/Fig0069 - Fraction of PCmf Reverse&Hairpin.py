...
if os.path.exists(join(figdata, code_id+'.pkl')):
    ...
    Data = DataFrameEstablish(variable_names = ['Fraction', 'Direction'], 
...
if os.path.exists(join(figdata, code_id+' [Hairpin].pkl')):
    ...
    HPData = DataFrameEstablish(variable_names = ['Fraction', 'Direction'], 
...
if os.path.exists(join(figdata, code_id+' [Field Num].pkl')):
    ...
    FN_Data = DataFrameEstablish(variable_names = ['Field Number', 'Direction'], 
...
if os.path.exists(join(figdata, code_id+' [Hairpin [Field Num]].pkl')):
    ...
    FN_HPData = DataFrameEstablish(variable_names = ['Field Number', 'Direction'], 
                                   f = f4, function = AverageFieldNumber_Reverse_Interface, 
                                   file_name = code_id + ' [Hairpin] [Field Num]', 
                                   behavior_paradigm = 'HairpinMaze')
    
print_estimator(FN_Data['Field Number'][np.where(FN_Data['Direction'] == 'cis')[0]])
print_estimator(FN_Data['Field Number'][np.where(FN_Data['Direction'] == 'trs')[0]])
print_estimator(FN_HPData['Field Number'][np.where(FN_HPData['Direction'] == 'cis')[0]])
print_estimator(FN_HPData['Field Number'][np.where(FN_HPData['Direction'] == 'trs')[0]], end='\n\n')
    
print_estimator(Data['Fraction'][np.where(Data['Direction'] == 'cis')[0]])
print_estimator(Data['Fraction'][np.where(Data['Direction'] == 'trs')[0]])
print_estimator(HPData['Fraction'][np.where(HPData['Direction'] == 'cis')[0]])
print_estimator(HPData['Fraction'][np.where(HPData['Direction'] == 'trs')[0]])